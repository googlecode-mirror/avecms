...
function checkSeePerm($id)
{
    ...
    if (!empty($navigations[$id]->Gruppen))
    {
...
function ave_htmlspecialchars($string = '')
{
	if (!is_numeric($string) && is_string($string))
	{
		$string = addslashes(htmlspecialchars_decode(stripslashes($string), ENT_QUOTES));
	}
