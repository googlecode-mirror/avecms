	...
	<div class="block">
		<div class="mod_shop_newprod_box">
			<table width="100%"  cellpadding="0" cellspacing="0">
									...
									<ul>
										{foreach from=$item->sub item=sub name=subs}
											{if $smarty.request.categ}
												<li><a class="categtitle_n" title="{$sub->visible_title|escape:'html'}" href="{$sub->dyn_link}">{$sub->visible_title|escape:'html'} </a></li>
												{*... {$sub->data} *}
											{else}
												{if ($smarty.foreach.subs.iteration <= $maxsubs)}
													<li><a class="categtitle_n" title="{$sub->visible_title|escape:'html'}" href="{$sub->dyn_link}">{$sub->visible_title|escape:'html'} </a></li>
													{if !$smarty.foreach.subs.last}{/if}
												{elseif $showalllink!=1}
													<li><a class="categtitle_n" href="{$item->dyn_link}">{#ShowAll#}</a></li>
				...
				{if $smarty.foreach.dl.iteration % $cols == 0}</tr>
				<tr>{/if}
				{/foreach}
				</tr>
			</table>
