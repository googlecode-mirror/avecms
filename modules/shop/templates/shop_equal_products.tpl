	...
	<div class="mod_shop_container_1">
		...
		<div class="grid_8" >
			...
			{if $i->ZeigeNetto==1 && $i->Preis_USt>0}
				<p>
					{#IncludeMwSt#} {num_format val=$i->Preis_Netto_Out} {$Currency} <!-- {#Netto#} -->+ {#InVatOnce#} {num_format val=$i->Preis_USt} {$Currency}
					{if $i->PreisDiff>0}
			...
			<div class="mod_shop_price_box">
				{if $i->PreisDiff > 0}
					...
					<div class="mod_shop_price_new">
						{num_format val=$i->Preis} {$Currency}<img src="/templates/ave/modules/shop/shop_scell.gif" width="27" height="27" alt="Старая цена {$i->PreisListe} {$Currency}" />
					</div>
				{else}
					<!-- Стоимсоть -->
