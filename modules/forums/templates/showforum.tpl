				...
				<table width="100%" cellpadding="2">
					<tr>
						...
						<form action="index.php?module=forums&amp;show=showforum&amp;fid={$smarty.get.fid}" method="post">
							...
							<select name="sort">
								<option value="desc" {if $smarty.post.sort == 'desc' || $smarty.get.sort == 'desc'}selected{/if}>{#Desc#}</option>
								<option value="asc" {if $smarty.post.sort == 'asc' || $smarty.get.sort == 'asc'}selected{/if}>{#Asc#}</option>
							</select>
