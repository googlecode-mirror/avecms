  ...
  <tr>
	...
	<table width="100%" border="0" cellpadding="4" cellspacing="1" class="forum_tableborder">
        ...
        <tr class="{cycle name='' values='forum_post_first,forum_post_second'}">
          <td>
		  <a title="{#UserPopInsertInf#}" class="forum_links_small" href="javascript:void(0);" onClick="unametofield('{$post->BenutzerName}')">{$post->BenutzerName|truncate:40}</a>		  </td>
          <td>
		  <a title="{#UserPopInsertInf#}" class="forum_links_small" href="javascript:void(0);" onClick="unametofield('{$post->BenutzerId}')">{$post->BenutzerId}</a></td>
        </tr>
        {/foreach}
      </table>
	  <br />
