		...
		<td>
			<div align="right">
				...
				<form method="get">
					<select name="pp">
						{section name=pps loop=76 step=5 start=15}
							<option value="{$smarty.section.pps.index}" {if $smarty.request.pp == $smarty.section.pps.index}selected="selected"{/if}>{$smarty.section.pps.index} {$lang.eachpage}</option>
						{/section}
					</select>
					<input type="submit" class="button" value="{#ButtonGo#}">
					<input type="hidden" name="module" value="forums" />
