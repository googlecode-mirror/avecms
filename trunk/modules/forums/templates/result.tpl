...
<a class="forum_links" href="{$topic.link}&amp;high={$smarty.get.pattern}">{$topic.title|stripslashes}</a><br />
	  {#PageNameForums#}: <a href="{$topic.forumslink}">{$topic.f_title}</a>		</td>
		...
		&nbsp;
		{else}
		<a href="javascript:;" class="forum_links_small" onclick="window.open('index.php?module=forums&amp;show=showposter&amp;tid={$topic.id}&amp;pop=1&amp;theme_folder={$theme_folder}', 'Poster', 'toolbar=no,scrollbars=yes,resizable=yes,width=400,height=400')">
