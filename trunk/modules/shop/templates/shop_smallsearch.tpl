...
<form method="post" action="{$ShopStartLink}">
	<label>{#ProductSArtTitle#}</label>
	<input class="inputbox" name="product_query" id="pq" type="text" value="{$smarty.post.product_query|stripslashes|escape:html|default:''}" />
	...
	<input class="inputbox priceinput" type="text" name="price_start" id="pfpt" value="{$smarty.post.price_start|stripslashes|escape:html|default:'0.01'}" /> -

	<input class="inputbox priceinput" type="text" name="price_end" value="{$smarty.post.price_end|stripslashes|escape:html|default:'999999.99'}" /> {$Currency}
	...
	<select class="inputbox" name="recordset" id="rs">
		{section name=recordset loop=25 step=5}
			...
			{if $smarty.request.recordset == ''}
				{if $smarty.section.recordset.index+5==10}{assign var=sel value='selected'}{/if}
			{else}
				{if $smarty.section.recordset.index+5==$smarty.request.recordset}{assign var=sel value='selected'}{/if}
			{/if}
